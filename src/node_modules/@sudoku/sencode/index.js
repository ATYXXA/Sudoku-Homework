...
function isValidUrl(string) {
    const urlPattern = new RegExp('^(https?:\\/\\/)?' 					+ // 协议必须是HTTP或HTTPS
        '((([a-zA-Z\\d]([a-zA-Z\\d-]*[a-zA-Z\\d])*)\\.)+[a-zA-Z]{2,}|'	+ // 域名（example.com）或 IPv4 地址（192.168.1.1）
        '((\\d{1,3}\\.){3}\\d{1,3}))' 									+ // 或者IPv4地址
        '(\\:\\d+)?(\\/[-a-zA-Z\\d%_.~+]*)*'							+ // 可选的端口号、路径
        '(\\?[;&a-zA-Z\\d%_.~+=-]*)?'									+ // 可选的查询字符串
        '(\\#[-a-zA-Z\\d_]*)?$', 'i'); 									  // 片段定位符（# 部分）
	const parsedUrl = new URL(string);

	// 通过正则表达式 urlPattern.test(string) 判断字符串是否符合基本的 URL 格式。
...
function extractDomainAndBd(url) {
	...
	// 使用 URL 构造器解析传入的 URL 字符串 url
    const parsedUrl = new URL(url);

	// 提取 URL 的域名部分
    const domain = parsedUrl.hostname;

	// 使用 URLSearchParams 获取查询参数 bd 的值
