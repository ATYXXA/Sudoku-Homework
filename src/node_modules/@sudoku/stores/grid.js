...
function getHintCells(grid) {
	...
	const factory = new SudokuStrategyFactory();
	// const strategy = factory.createStrategy('x-wing');
	const strategy = factory.createStrategy('rectangle-elimination');

	// 保存原始网格的副本
	let originalGrid = JSON.stringify(grid);

	// 首先创建一个新数组，通过深拷贝的方式复制 grid 数组
	let newGrid = JSON.parse(JSON.stringify(grid));
	// 解题，将新数组作为 solve 的参数传入
	const solvedSudoku = strategy.solve(newGrid);
	...
	for (let x = 0; x < 9; x++) {
		for (let y = 0; y < 9; y++) {
			if (grid[x][y] === 0 && solvedSudoku[x][y]!== 0) {
				// console.log("x和y分别是",x,"和",y);
				uniqueCells.push({ x, y, value:solvedSudoku[x][y]});
			}
		}
	}
